...
class QClients(MainQueryHandler):
    ...
    def get_visible_data():
        return [[i.brand, ' '.join([i.Fname, i.Iname, i.Oname]), i.phone, i.email, i.comment] for i in
                Clients.query.all()]
    ...
    def add_row(form):
        ...
        for rate in form.rates:
            try:
                db.session.add(Clients_rates_inner(rates_and_products_id=tmp_rates_and_products.id,
                                                   product_group_id=rate.product_group_id.data,
        ...
        for product in form.products:
            try:
                db.session.add(Clients_products_inner(rates_and_products_id=tmp_rates_and_products.id,
                                                      product_id=product.product_id.data, quanity=product.quanity.data))
                db.session.commit()
            except sqlalchemy.exc.IntegrityError:
                print("Clients try except!!!!!!")  # !!!!!!!!!!
    ...
    def form(data=None, empty=False):
        ...
        for i, product in enumerate(Products.query.all()):
            ...
            if len(form.products.clm_names) <= 0:
                form.products.clm_names = [i.label.text for i in
                                           [cl_product.checkbox, cl_product.product_name, cl_product.quanity]]
...
class QPrepared_cars(MainQueryHandler):
    ...
    def form(data=None, empty=False):
        res = FPrepared_cars()
        res.car_id.choices = [(i.id, i.number + " (" + Car_types.query.get(i.type_id).car_type + ")") for i in
                              Car_numbers.query.all()]
...
class QFinances(MainQueryHandler):
    ...
    def get_visible_clm_names():
        return ['Клиент Маркировкa(Маркировкa)', 'Сумма по машинам в пути($)', 'К оплате($)', 'Оплачено($)', 'Итого($)',
                'Дней с разгрузки']

    def get_visible_data():
        return [[Clients.query.get(i.clients).brand, i.Progress_sum, i.Now_sum, i.Paid, i.Overall, i.Days] for i in
                Finances.query.all()]

    def add_row(form):
        tmp = Finances(clients=form.clients.data, Progress_sum=form.Progress_sum.data, Now_sum=form.Now_sum.data,
                       Paid=form.Paid.data, Overall=form.Overall.data, Days=form.Days.data)
        try:
            db.session.add(tmp)
            db.session.commit()
        except sqlalchemy.exc.IntegrityError:
...
class QExchange_rates(MainQueryHandler):
    ...
    def add_row(form):
        tmp = Exchange_rates(date=form.date.data, currency_dollar=form.currency_dollar.data,
                             currency_euro=form.currency_euro.data, comment=form.comment.data, author=form.author.data)
        try:
            db.session.add(tmp)
            db.session.commit()
        except sqlalchemy.exc.IntegrityError:
